...
void _rtpv_process_nalu(rtpv_s *rtpv, const uint8_t *data, size_t size, uint32_t pts, bool marked, rtp_callback_f callback);
...
void rtpv_wrap(rtpv_s *rtpv, const frame_s *frame, rtp_callback_f callback) {
	...
	while (true) { // Find and iterate by nalus
		...
		if (last_offset >= 0) {
			...
			_rtpv_process_nalu(rtpv, data, size, pts, false, callback);
		}
	...
	if (last_offset >= 0) {
		...
		_rtpv_process_nalu(rtpv, data, size, pts, true, callback);
	}
}
